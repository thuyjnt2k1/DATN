...
df_links = pd.read_csv('remove_duplicate_link.csv')
df_nodes = pd.read_csv('final_node.csv', index_col='id')
print(df_nodes)
df_author = pd.read_csv('final_author.csv', index_col='ner_id')
# df_author = df_author.astype({'orcid': 'str', 'email': 'str', 'affiliation': 'str'})
df_paper = pd.read_csv('final_paper.csv', index_col='ner_id')
...
grouped = df_nodes.groupby(['type']).count()

# print(grouped)
for id, link in df_links.iterrows():
  if(int(link['from']) in G.nodes and int(link['to']) in G.nodes ):
